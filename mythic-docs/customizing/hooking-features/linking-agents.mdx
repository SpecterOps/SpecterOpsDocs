...
<Info>
This also comes into play when trying to connect to a newly executed payload that hasn't gone through the checkin process with Mythic yet to get registered as a Callback.
</Info>
...
<Frame>
<img src="../images/version-3.3/image-1-52.png"/>
</Frame>
...
Once you've selected a host, the `Payload` dropdown will populate with the associated payloads that Mythic knows are on that host (this will also contain callbacks with p2p c2 profiles). These payloads are in two main groups:
...
This payload simply acts as a template of information so that you can select the final piece. The payloads listed are also the only ones that have a p2p profile associated with them.

#### What if your host/payload isn't listed?

Select the green `Register New` button, manually specify which host your payload lives on, then select from the dropdown the associated payload that was used. Then click `Confirm`. Now Mythic is also tracking that the selected payload lives on the indicated host. You can continue with the host/payload/c2\_profile dropdowns like normal.

<Frame>
<img src="../../images/version-3.3/image-1-53.png"/>
</Frame>
